...
/*
  ...
  --  Author        Phoungpaka.H
  --  DATE          17/09/2021
  --  Purpose       燹膾∫輿拇 TTB Automatic payment reconciliation from Fast Quote and OPUS
   ...
   IF  r_gnr_quote_data.COVER_PERIOD_YEAR = '1' THEN --1 Year
    ...
    ELSE
        ...
        /*IF v_cover_year1 =1 THEN
           r_gnr_quote_data.ref2 :=r_gnr_quote_data.Y1_PAYREF_NUM2; --if 篩菠茹よ卅 Default 1 Year
        END IF;
...
END;
----------------------------------------------------------------------------------------------------
--燹膾∫輿拇 error from insert gnr_quote_data
